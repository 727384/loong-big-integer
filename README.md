...
~~**部分代码使用AI辅助完成。**~~ 其实并没有，AI只起辅助作用，没有参与实际编写。
...
使用`struct lBI`创建大整数变量。

几乎所有函数都带有`lBI_`前缀，但`int_to_lBI()`、`str_to_lBI()`、`input_lBI()`和`print_lBI()`除外。
...
更新记录（`lbi.h`）
---
2026/9/6 UPD：修复了一些bug。

2026/8/30 UPD：新增`to_c_str()`成员函数和`loong::lBI_to_c_str()`函数。再次兼容 C++ 98。

2026/8/28 UPD：修复了一些bug。

2026/8/27 UPD：修复了一个bug。

2026/8/25 UPD：无用的更改。

2026/7/30 UPD：新增`loong::atan()`、`loong::atan2()`和`loong::atanh()`，兼容以前的代码及 C++ 98。
...
2025/11/29 UPD：现在四则运算也支持函数形式了。

2025/10/24 UPD：初次上传代码。

更新记录（`lbi_c.h`）
---
2026/8/30 UPD：修复了一些bug。

2026/8/28 UPD：初次上传代码。
